...
import type { Card, GameState, Move, PlayerId, Suit, TrickState } from './types';
import { CARD_RANK } from './constants';
...
export function applyMove(state: GameState, move: Move): GameState {
  ...
  } as Record<PlayerId, typeof state.players.human>;

  // Round ends when no cards remain anywhere
  const roundEnded =
    deck.length === 0 &&
    newPlayers.human.hand.length === 0 &&
    newPlayers.cpu.hand.length === 0;
